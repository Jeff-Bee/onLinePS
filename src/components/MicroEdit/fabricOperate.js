...
let fabricObject = function (that,id="canvas") {
    ...
    /* 创建图片 */
    const _createImage = function (url, imgConfig, centerObject = false, hasControls = true) {
        return new Promise((resolve, reject)=>{
            try{
                let imgInstance = new fabric.Image.fromURL(url, function (img) {
                    img.set(imgConfig);
                    img.set({
                        scaleX: imgConfig.bboxWidth ? (parseInt(imgConfig.bboxWidth) / img.width) : 1,
                        scaleY: imgConfig.bboxHeight ? (parseInt(imgConfig.bboxHeight) / img.height) : 1,
                        realWidth: imgConfig.bboxWidth ? parseInt(imgConfig.bboxWidth) : parseInt(img.width),
                        realHeight: imgConfig.bboxHeight ? parseInt(imgConfig.bboxHeight) : parseInt(img.height),
    ...
    let updateCanvasState = function () {
        if (_config.undoStatus == false && _config.redoStatus == false) {
            ...
            if (_config.currentStateIndex == _config.canvasState.length - 1 && _config.currentStateIndex == 0) {
              return;
            }
            if (isInitialStatus){
    ...
    let updateUnRedoBtnStatus=function(){
        ...
        //回退按钮状态更新
        if (_config.canvasState.length > _config.currentStateIndex + 1 && _config.canvasState.length > 1) {
    ...
    let undoFinish = function () {
        ...
        //gif动图微编辑
        that.initialSuccessCallback&&that.initialSuccessCallback(getNewEditLevelList());
        updateUnRedoBtnStatus();
    }
    ...
    let canvasLoadFromJsonTxtLoadFont=function(o,object){
        if(object.isType('i-text')&&o.fontFamily==""&&o.realFontFamily!="")
        {
